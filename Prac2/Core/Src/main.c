...
int main(void)
{
  ...
  {
	  write_to_address(start_address+i,patterns[i]);
  }
  /* USER CODE END 2 */
  ...
  while (1)
  {
	...
	uint16_t readIDR = GPIOA->IDR;
	if (((readIDR | GPIO_IDR_0) != readIDR) && one_second) // The bit will go from 1 to 0 if the switch is pressed
	{
		delay(button_delay); //Debounce delay
		__HAL_TIM_SET_AUTORELOAD(&htim16,500-1);
		one_second = 0;
	}
	else if ((readIDR | GPIO_IDR_0) != readIDR)
	{
		delay(button_delay); //Debounce delay
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t read_value = read_from_address(start_address+p_index);
	if (read_value != patterns[p_index])
	{
